...
import (
	...
	"github.com/victorgomez09/viraas/internal/cli/config"
...
var RootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if flags.GetVersionFlag(cmd) {
			fmt.Printf("Viraas CLI version %s, build %s\n", metadata.Version, metadata.BuildHash)
...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.viraas.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".viraas" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".viraas")
