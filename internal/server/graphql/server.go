...
import (
	...
	"github.com/victorgomez09/viraas/internal/server/gqlgen"
	"github.com/victorgomez09/viraas/web"
)
...
func (s *graphqlServer) ServeGraphql() error {
	...
	r.Handle("/playground", playground.Handler("Viraas", "/graphql"))
	r.Get("/*", web.Handler("/"))

	s.logger.I("Viraas server started at :%d", s.port)
...
func (s *graphqlServer) tokenAuthMiddleware(next http.Handler) http.Handler {
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		...
		if authHeader == "" {
			writeGraphqlError(
				rw,
				"Authorization header required, but was not passed. See https://victorgomez09.github.io/viraas/authorization for more details",
		...
		if !strings.HasPrefix(authHeader, headerPrefix) {
			writeGraphqlError(
				rw,
				"Authorization header format incorrect, must be \"Bearer <token>\". See https://victorgomez09.github.io/viraas/authorization for more details",
				http.StatusUnauthorized,
			)
			return
		}
