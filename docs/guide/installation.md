...
:::tip Machine OS
Before starting, make sure the machine you're installing the server on is one of the [supported architectures](https://hub.docker.com/r/vira/viraas/tags).
...
1. Run the server's Docker image
   ...
   - **`-v /var/run/docker.sock:/var/run/docker.sock`** - Bind a special volume that lets Viraas talk to the docker daemon running on the host machine so it can do things like start/stop apps
   - **`-v $HOME/.viraas:/data/viraas`** - Bind a volume so Viraas can persist it's configuration and remember what apps have been setup if the container is stopped and restarted
   - [`vira/viraas`](https://hub.docker.com/r/vira/viraas) - The name of the docker image to run. To use a different version of the image, include a `:tag` suffix
     :::
...
<script setup>
  ...
  const binaries = {
    linux: "/usr/local/bin/viraas",
    windows: "C:\\\"Program Files\"\\viraas\\viraas.exe",
    darwin: "/usr/local/bin/viraas",
  }
...
The CLI is a standalone binary, so just download the [latest version from GitHub](https://github.com/victorgomez09/viraas/releases?q=cli), verify it's checksum, and add it to your path.
...
<span class="token function">curl</span> -L -O https://github.com/victorgomez09/viraas/releases/download/cli-v{{cliVersion}}/viraas-cli-{{os}}-{{arch}}
<span class="token function">curl</span> -L -O https://github.com/victorgomez09/viraas/releases/download/cli-v{{cliVersion}}/viraas-cli-{{os}}-{{arch}}.sha256
<span></span>
<span class="token comment"># Verify</span>
<span>sha256sum --check viraas-cli-{{os}}-{{arch}}.sha256</span>
<span></span>
<span class="token comment"># Add to Path</span>
<span class="token function">sudo mv</span> viraas-cli-{{os}}-{{arch}} {{binary}}
